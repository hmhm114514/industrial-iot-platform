package com.practice.visual.config;
...
public class SecurityConfig implements WebMvcConfigurer {
    ...
    @Override public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(new TokenInterceptor(token)).addPathPatterns("/api/**").excludePathPatterns("/api/auth/login");
    }
}

class TokenInterceptor implements org.springframework.web.servlet.HandlerInterceptor {
    private final String token;
    TokenInterceptor(String token) { this.token = token; }
