...
public final class CrudSupport {
    private CrudSupport() {}
    public static <T> List<T> search(JpaRepository<T,Long> dao, String keyword) { List<T> all=dao.findAll(); if (keyword==null || keyword.isBlank()) return all; String kw=keyword.toLowerCase(); return all.stream().filter(x -> text(x).contains(kw)).toList(); }
